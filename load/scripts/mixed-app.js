...
function getApplicationId(cookieHeader) {
  ...
  recordResponseMetrics(response, 'applications', 'dashboard', response.status === 200);
...
export function setup() {
  ...
  recordResponseMetrics(signInResponse, 'auth-sign-in', 'dashboard', signInResponse.status === 200);
  ...
  const eventTemplates = [
    ...
    },
    {
      ...
      properties: {
        ...
        buttonId: ['upgrade-plan', 'create-report', 'connect-integration'][__ITER % 3],
    ...
    },
    {
      ...
      properties: {
        ...
        productId: ['starter-monthly', 'pro-monthly', 'enterprise-addon'][__ITER % 3],
        quantity: 1 + (__ITER % 2),
...
export function dashboardReads(data) {
  ...
  const responses = http.batch([
    [
      ...
      null,
      { headers: { Cookie: data.cookieHeader }, tags: { endpoint: 'quality', flow: 'dashboard', generator: GENERATOR_ID } },
    ],
    [
      'GET',
      `${BASE_URL}/api/schemas?applicationId=${data.applicationId}`,
      null,
      { headers: { Cookie: data.cookieHeader }, tags: { endpoint: 'schemas', flow: 'dashboard', generator: GENERATOR_ID } },
    ],
    [
      'GET',
      `${BASE_URL}/api/segments?applicationId=${data.applicationId}`,
      null,
      { headers: { Cookie: data.cookieHeader }, tags: { endpoint: 'segments', flow: 'dashboard', generator: GENERATOR_ID } },
    ],
    [
      'POST',
      `${BASE_URL}/api/query`,
      ...
      { headers: cookieHeaders, tags: { endpoint: 'query', flow: 'dashboard', generator: GENERATOR_ID } },
    ],
  ]);
