...
import { GET as listFunnelsRoute, POST as createFunnelRoute } from '@/app/api/funnels/route';
import {
...
describe('funnels api routes', () => {
  ...
  it('lists funnels for an authenticated viewer', async () => {
    ...
    const response = await listFunnelsRoute(
      new NextRequest('http://localhost:3000/api/funnels?applicationId=app-1&q=signup'),
    );
  ...
  it('fetches a funnel by id and returns 404 when missing', async () => {
    ...
    vi.mocked(getFunnel)
      .mockResolvedValueOnce({ id: 'funnel-1', applicationId: 'app-1' } as never)
      .mockResolvedValueOnce(null);

    const success = await getFunnelRoute(new NextRequest('http://localhost:3000/api/funnels/funnel-1'), {
      params: Promise.resolve({ id: 'funnel-1' }),
    });
    expect(success.status).toBe(200);

    const missing = await getFunnelRoute(new NextRequest('http://localhost:3000/api/funnels/missing'), {
